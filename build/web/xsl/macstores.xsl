        ...
        <xsl:element name="products">
            ...
            <xsl:for-each select="$products">
                <xsl:element name="product">
                    <title><xsl:value-of select=".//h4[@class='mkdf-product-list-product-title']"/></title>
                    <price><xsl:value-of select="translate(normalize-space((.//span[@class='woocommerce-Price-amount amount'])[last()]), '.', '')"/></price>
                    <image><xsl:value-of select=".//img/@src"/></image>
                    <url><xsl:value-of select=".//a[@class='mkdf-woo-product-info-holder']/@href"/></url>
                </xsl:element>
            </xsl:for-each>
        </xsl:element>
    </xsl:template>
