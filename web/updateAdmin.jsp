...
<%@include file="fragment/adminDocumentFragment.jsp" %>
...
<c:choose>
    <c:when test="${txtUsername}">
        <x:set var="txtUsername" select="$adminDocument/username"/>
    </c:when>
    <c:otherwise>
        <x:set var="txtUsername" select="$txtUsername"/>
    </c:otherwise>
</c:choose>
