        ...
        <xsl:element name="products">
            ...
            <xsl:for-each select="$products">
                <xsl:element name="product">
                    <title>
                        <xsl:value-of select=".//a[@class='TitleProduct'][2]"/>
                    </title>
                    <price>
                        <xsl:value-of select="translate(normalize-space(substring-before(substring-after(.,'Giá:'),' vn')), '.', '')"/>
                    </price>
                    <image>
                        <xsl:value-of select=".//img/@src"/>
                    </image>
                    <url>
                        <xsl:value-of select=".//a[@class='TitleProduct']/@href"/>
                    </url>
                </xsl:element>
            </xsl:for-each>
        </xsl:element>
